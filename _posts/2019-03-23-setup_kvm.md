...
```shell
sudo pacman -Syu
sudo pacman -S qemu libvirt dmidecode bridge-utils ebtables dnsmasq
```

`openbsd-netcat` 和 `gnu-netcat` 互相冲突，安装任意一个就行
...
> 如果不能出现 `error: Unable to create bridge virbr0: Package not installed`， 那么重启后好像就可以正常启动了？
